...
// clementinaGPIOEmulatorConsoleConfig holds the configuration for creating a new Clementina emulator console.
// It embeds the base EmulatorConsoleConfig and adds a reference to the Clementina emulator instance.
type clementinaGPIOEmulatorConsoleConfig struct {
	terminal.BaseEmulatorConsoleConfig
	emulator *clementinaGPIOEmulator
}

// clementinaGPIOEmulatorConsole represents the main console interface for the Clementina 6502 emulator.
// It manages the terminal UI, window layout, and user interactions for the emulator.
type clementinaGPIOEmulatorConsole struct {
...
// newClementinaGPIOEmulatorConsole creates a new instance of the Clementina emulator console.
...
func newClementinaGPIOEmulatorConsole(config clementinaGPIOEmulatorConsoleConfig) *clementinaGPIOEmulatorConsole {

	console := &clementinaGPIOEmulatorConsole{
	...
	wm.AddWindow("mia", ui.NewMiaWindow(computer.chips.mia))
...
func (c *clementinaGPIOEmulatorConsole) initializeMainGrid() {
...
func initializeBusWindow(computer *ClementinaGPIOComputer, busWindow *ui.BusWindow) {
...
func (c *clementinaGPIOEmulatorConsole) initializeLayout() {
...
func (c *clementinaGPIOEmulatorConsole) ShowGotoForm() {
	wm := c.windowManager
