...
// NewClementinaGPIOComputer creates and initializes a new instance of the Clementina 6502 computer emulation.
...
func NewClementinaGPIOComputer() (*ClementinaGPIOComputer, error) {
	chips := &chips{
		...
		mia:      pico.NewMiaConnector("gpiochip0"),
	}
	...
	mappers := mappers{
		...
		miaAddressBus: mapperFunctions[uint8, uint16]{
	...
	miaAddressBus := buses.New8BitMappedBus(
		[]buses.Bus[uint16]{addressBus},
		mappers.miaAddressBus.MapToSource,
		mappers.miaAddressBus.MapFromSource,
	)
	...
	circuit := &circuit{
		...
		portBBus:      portBBus,
		picoHiRAME:    buses.NewStandaloneLine(true), // TODO: Must default to true
		vcc:           buses.NewStandaloneLine(true),
		ground:        buses.NewStandaloneLine(false),
		miaAddressBus: miaAddressBus,
	}
	...
	// MIA Connections
	chips.mia.AddressBus().Connect(circuit.miaAddressBus)
	chips.mia.DataBus().Connect(circuit.dataBus)
	chips.mia.HiRAMEnable().Connect(circuit.picoHiRAME)
	//chips.mia.Reset().Connect(circuit.cpuReset)
	chips.mia.WriteEnable().Connect(chips.oeRWSync.RW())
	chips.mia.OutputEnable().Connect(chips.oeRWSync.OE())
	chips.mia.HiRAMCS().Connect(chips.csLogic.HiRAME())
	chips.mia.IO0CS().Connect(chips.csLogic.IOOE().GetBusLine(0))
